...
async function submitWordSuggestions() {
  ...
  let wordsToSend = candidateWords;
  ...
  try {
    ...
    const fiveCellWords = louisResults.filter(r => r.cellCount === LOUIS_TARGET_CELL_COUNT);
    const rejectedWords = louisResults.filter(r => r.cellCount !== LOUIS_TARGET_CELL_COUNT);

    if (fiveCellWords.length === 0) {
      const summary = rejectedWords
        .map(r => `${r.word} (${r.cellCount} cell${r.cellCount === 1 ? "" : "s"})`)
        .join(", ");
      setSuggestFeedback(`None of those are 5 braille cells in Grade 2 UEB: ${summary}.`);
      if (submitBtn) submitBtn.disabled = false;
      return;
    }

    wordsToSend = fiveCellWords.map(r => r.word);

    if (rejectedWords.length > 0) {
      const summary = rejectedWords.map(r => `${r.word} (${r.cellCount})`).join(", ");
      setSuggestFeedback(`Sending ${wordsToSend.length}. Skipped non-5-cell: ${summary}.`);
    } else {
  ...
  formData.append("words", wordsToSend.join(" "));
  formData.append("wordCount", String(wordsToSend.length));
  ...
  try {
    ...
    if (response.ok) {
      setSuggestFeedback(
        wordsToSend.length === 1
          ? "Thanks! Your word suggestion was sent."
          : `Thanks! ${wordsToSend.length} word suggestions were sent.`
      );
      input.value = "";
    } else {
