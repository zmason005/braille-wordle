...
IMPL.lou = {
	...
	translateString: function(table, inbuf, backtranslate) {
		...
		// PATCHED (contractable): lou_translateString's inlen/outlen params
		// are counted in *widechars* (characters), not bytes. The original
		// code below reused the buffer's BYTE length (chars * charSize) for
		// both the malloc size AND these length params. On the UTF-32 build
		// that's off by 4x (2x on UTF-16), so liblouis was told the input
		// was up to 4x longer than it really was, and read/wrote past the
		// actual buffer into adjacent WASM heap memory -- producing
		// corrupted/garbage translations for some words, depending on what
		// happened to be sitting in nearby memory at call time.
		//
		// Fix: pass actual widechar counts (not byte counts) in strlen_ptr /
		// bufflen_ptr, and size the output buffer generously in case a
		// translation ever needs more cells/characters than the input had.
		var bytesPerChar = this.mem.getBufferLength(""); // (0+1)*charSize == charSize
		var inCharCount = inbuf.length;
		var outCharCapacity = (inCharCount * 2) + 1;

		var inbuff_ptr = this.mem.transfer(this.capi, inbuf);
		var outbuff_ptr = this.capi._malloc(outCharCapacity * bytesPerChar);

		var bufflen_ptr = this.capi._malloc(4);
		var strlen_ptr = this.capi._malloc(4);

		this.capi.setValue(bufflen_ptr, outCharCapacity, "i32");
		this.capi.setValue(strlen_ptr, inCharCount, "i32");
